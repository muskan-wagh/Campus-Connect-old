        ...
        <div className="pb-12 text-slate-900">
            ...
            <div className="space-y-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {stats.map((stat) => (
                        <div key={stat.name} className="bg-white border border-slate-100 p-8 rounded-[2.5rem] relative overflow-hidden group shadow-xl shadow-slate-200/50 hover:shadow-slate-950/10 transition-all duration-500">
                            <div className="relative z-10">
                                <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.3em] mb-3">{stat.name}</p>
                                <div className="flex items-baseline gap-3">
                                    <h2 className="text-5xl font-black tracking-tighter text-slate-900 group-hover:text-slate-950 transition-colors">{stat.value}</h2>
                                    <div className="w-2.5 h-2.5 rounded-full bg-slate-950 animate-pulse shadow-[0_0_10px_rgba(249,115,22,0.5)]"></div>
                                </div>
                            </div>
                            <div className="absolute -right-4 -bottom-4 w-28 h-28 bg-slate-50 rounded-full opacity-50 group-hover:scale-110 transition-transform duration-700 flex items-center justify-center p-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <div className="lg:col-span-2 space-y-8">
                        <div className="flex items-center justify-between border-b border-slate-100 pb-5">
                            ...
                            <Link href="/dashboard/admin/verify-clubs" className="text-[10px] font-black text-slate-950 uppercase tracking-widest hover:underline decoration-2 underline-offset-4 transition-all">Process All Nodes &rarr;</Link>
                        </div>
                        ...
                        <div className="bg-white border border-slate-100 rounded-[2.5rem] overflow-hidden shadow-xl shadow-slate-200/40">
                                ...
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {pendingClubs?.length > 0 ? pendingClubs.map(club => (
                                            <tr key={club.id} className="hover:bg-slate-50/30 transition-all group">
                                                <td className="px-10 py-8">
                                                    <div className="flex items-center gap-5">
                                                        <div className="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 group-hover:text-slate-950 group-hover:border-slate-200 font-black text-sm transition-all shadow-inner">
                                                            {club.name[0]}
                                                        </div>
                                                        <span className="font-bold text-slate-900 tracking-tight text-base group-hover:text-slate-950 transition-colors">{club.name}</span>
                                                    </div>
                                                </td>
                                                <td className="px-10 py-8">
                                                ...
                                                <td className="px-10 py-8 text-right">
                                                    <Link href={`/dashboard/admin/clubs/${club.id}`} className="px-8 py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-950 transition-all shadow-md hover:shadow-slate-950/20">Review</Link>
                                                </td>
                                            </tr>
                                        )) : (
                                            <tr>
                            ...
                            <div className="md:hidden divide-y divide-slate-100">
                                {pendingClubs?.length > 0 ? pendingClubs.map(club => (
                                    <div key={club.id} className="p-8 hover:bg-slate-50/30 transition-all">
                                        ...
                                        <Link href={`/dashboard/admin/clubs/${club.id}`} className="block w-full text-center px-8 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-950 transition-all shadow-lg shadow-slate-200">
                                            Review Interface
                                        </Link>
                                    </div>
                                )) : (
                    ...
                    <div className="space-y-10">
                        ...
                        <div className="bg-white border border-slate-50 p-10 rounded-[3rem] shadow-xl shadow-slate-100 relative overflow-hidden">
                            <div className="absolute -top-12 -right-12 w-32 h-32 bg-slate-950/5 rounded-full blur-2xl"></div>

                            <div className="space-y-8 relative z-10">
                                {[
                                    { name: 'Total Nodes', count: totalClubs, color: 'bg-slate-950', path: '/dashboard/admin/all-clubs' },
                                    ...
                                    <Link href={item.path} key={i} className="flex items-center justify-between group cursor-pointer border-b border-slate-50 pb-5 last:border-0 last:pb-0">
                                        <div className="flex items-center gap-5">
                                            ...
                                            <span className="font-black text-slate-500 text-[11px] group-hover:text-slate-950 transition-colors uppercase tracking-[0.2em]">{item.name}</span>
                                        </div>
                                        <span className="text-sm font-black text-slate-900 group-hover:text-slate-950 transition-colors">{item.count}</span>
                                    </Link>
                                ))}
                            </div>
                            <Link href="/dashboard/members" className="w-full mt-12 py-5 bg-slate-900 text-white rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest hover:bg-slate-950 block text-center transition-all shadow-lg hover:shadow-slate-950/20 active:scale-95">
                                MANAGE SYSTEM USERS
                            </Link>
                        </div>
